...
function resolveErrorCode(status: number, message: string): string {
  ...
  if (message.includes('AGENT_NOT_FOUND')) return 'AGENT_NOT_FOUND';
  if (message.includes('AGENT_ALREADY_SUBSCRIBED')) return 'AGENT_ALREADY_SUBSCRIBED';
  if (message.includes('SUBSCRIPTION_NOT_FOUND')) return 'SUBSCRIPTION_NOT_FOUND';
  if (message.includes('INVOICE_NOT_FOUND')) return 'INVOICE_NOT_FOUND';
