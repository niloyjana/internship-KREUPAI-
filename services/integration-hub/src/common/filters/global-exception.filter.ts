...
function resolveErrorCode(status: number, message: string): string {
  // Integration-specific codes
  if (message.includes('CONNECTION_NOT_FOUND')) return 'CONNECTION_NOT_FOUND';
  if (message.includes('PROVIDER_NOT_SUPPORTED')) return 'PROVIDER_NOT_SUPPORTED';
  if (message.includes('OAUTH_CALLBACK_FAILED')) return 'OAUTH_CALLBACK_FAILED';
  if (message.includes('CONNECTION_TEST_FAILED')) return 'CONNECTION_TEST_FAILED';
  if (message.includes('CONNECTION_ALREADY_EXISTS')) return 'CONNECTION_ALREADY_EXISTS';
